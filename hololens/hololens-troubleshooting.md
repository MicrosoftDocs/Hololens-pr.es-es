...
description: Manténgase al día de las soluciones más comunes para solucionar HoloLens problemas de dispositivos y técnicas de solución de problemas.
author: evmill
ms.author: v-evmill
ms.date: 10/7/2021
ms.prod: hololens
...
Hay un problema conocido conocido en el que cuando el dispositivo alcanza el 18 % de batería, se apagará inesperadamente. Se trata de un problema de software, no de hardware ni de batería, por lo que no debe intercambiar dispositivos por ello. Si no está seguro de si el problema coincide con este error, haga lo siguiente:
...
Si no puede actualizar el Remote Assist a una compilación más reciente, pruebe lo siguiente.
...
Algunos clientes han notificado un problema por el Microsoft Edge no se puede iniciar. Para estos clientes, el problema persiste a través del reinicio y no se resuelve con Windows actualizaciones de aplicaciones. Si experimenta este problema y ha confirmado que [Windows](hololens-updates.md#manually-check-for-updates)está actualizado, envíe un error desde la aplicación [Centro de opiniones](hololens-feedback.md) con la siguiente categoría y subcategoría: Instalar y actualizar > Descargando, instalando y configurando Windows Update.

No hay soluciones alternativas conocidas, ya que hasta ahora no hemos podido resolver la causa principal del problema. La presentación de un error Centro de opiniones ayudará a nuestra investigación. Se trata de un **problema conocido.**
...
- Unas extensiones de pestañas más marcadas y otras
- Gafas gruesas o de vello si impiden que el dispositivo vea los ojos
- Cierta fisicidad ocular, condiciones oculares o cáncer de ojos, como ojos estrechos, pestañas largas, amblyopia, nistagmus, algunos casos de LASIK u otras operaciones oculares
...
La información sobre el emulador HoloLens se encuentra en nuestra documentación para desarrolladores.  Obtenga más información sobre [la solución de problemas HoloLens emulador](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-hololens-emulator#troubleshooting).
...
Si Cortana no responde a los comandos de voz, asegúrese de Cortana está activado. En la lista Todas las aplicaciones, **seleccione Cortana**  >  **cuaderno de**  >    >  **menús Configuración** para realizar cambios. Para más información sobre lo que puede decir, consulte [Uso de la voz con HoloLens](hololens-cortana.md).
...
Si el visor tiene huellas digitales o marcas, use la limpieza de microfibras que se incluye con la HoloLens para limpiar el visor de forma transparente.
