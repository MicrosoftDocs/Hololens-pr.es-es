...
Si tiene problemas con la HoloLens, puede que desee intentar reiniciar o restablecer el dispositivo, o incluso volver a actualizar el dispositivo mediante la recuperación de dispositivos. Este artículo le guía por los pasos de recuperación recomendados en orden.

Si desea recuperar un HoloLens 2, consulte [Recuperación](hololens-recovery.md)de un HoloLens 2 , ya que ese proceso difiere.

> [!NOTE]
> Este artículo se centra en la HoloLens dispositivo y software. Si los hologramas no son **[](hololens-environment-considerations.md)** correctos, consulte consideraciones HoloLens entorno para obtener información sobre los factores que mejoran la calidad del holograma.

## <a name="restart"></a>Reiniciar

### <a name="do-a-safe-restart-by-using-cortana"></a>Realice un reinicio seguro mediante Cortana

La manera más segura de reiniciar el HoloLens es mediante Cortana, que suele ser lo primero que se debe intentar cuando se experimenta un problema con HoloLens.

> [!NOTE] 
> Cortana no está disponible en todos los dispositivos.
> - Cortana está disponible en todos los HoloLens (1.ª generación). 
> - Cortana está disponible en HoloLens 2 dispositivos en compilaciones anteriores a la actualización Windows Holograpic, versión 2004.

1. Active la HoloLens.
1. Asegúrese de que un usuario ha iniciado sesión y de que el dispositivo no está esperando una contraseña para desbloquearla.
...
1. Si sigue teniendo problemas,  presione el botón de encendido durante 4 segundos, hasta que todos los indicadores de batería se desvanezzcan y la pantalla deje de mostrar hologramas. Espere 1 minuto y presione de nuevo el **botón de** encendido para activar el dispositivo.

## <a name="go-back-to-a-previous-version---hololens-1st-gen"></a>Volver a una versión anterior: HoloLens (1ª generación)

En algunos casos, es posible que quiera volver a una versión anterior del software HoloLens software. Para ello, use la herramienta Windows Recuperación de dispositivos para restablecer el HoloLens a la versión anterior.
...
Si el HoloLens tiene un problema, intente restablecerlo al estado de fábrica. Este paso mantiene la versión del software Windows Holographic que está instalado en él y devuelve todo lo demás a la configuración de fábrica.
...
Si el dispositivo sigue teniendo un problema después del reinicio y el restablecimiento, puede usar una herramienta de recuperación en el equipo para volver a instalar el HoloLens operativo y el firmware.  
...
Puede instalar un nuevo sistema operativo en la HoloLens (1.ª generación) mediante Windows Device Recovery Tool. Antes de usar esa herramienta, vea si el reinicio o el restablecimiento del HoloLens corrige el problema.

El proceso de recuperación puede tardar un tiempo. Cuando haya terminado, se instalará la versión más reciente Windows software holográfico.
...
Si el HoloLens (1ª generación) no se detecta automáticamente, seleccione **Mi dispositivo no se detectó.** A continuación, siga las instrucciones para poner el dispositivo en modo de recuperación.
...
4. Abra la Windows Device Recovery Tool.
